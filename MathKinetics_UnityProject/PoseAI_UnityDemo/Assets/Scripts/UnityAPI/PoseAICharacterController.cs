...
namespace PoseAI
{
	...
	public class PoseAICharacterController : MonoBehaviour
	{
		...
		public GameObject StartTimeValueObj;
		...
		private void Awake()
		{
			...
			GameObject child = SoundEffects.transform.GetChild(3).gameObject;
			sound_effect = child.GetComponent<AudioSource>();
		}

		private void Start()
		{
			// When starting the gam we lok for the StartTimer
			StartTimeValueObj = GameObject.Find("Canvas/Start_Timer");
			startText = StartTimeValueObj.GetComponent<TextMeshProUGUI>();
        ...
        {
			//When you reset after Highscoretable
            if(level == 0)
            {
				StartTimeValueObj = GameObject.Find("Canvas/Start_Timer");
				startText = StartTimeValueObj.GetComponent<TextMeshProUGUI>();
		...
		private void Update()
		{
			if(_rigObj.IsStale())
            {
				Debug.LogWarning("poseai network connection lost");
            }

			//pause when in highscore
			if(SceneManager.GetActiveScene().buildIndex == 0) { 
			

			// must also be changed in scripts: "playermovement" and "moveoffsetgroundmat"
			if (StartTimeValue > 0)
			{
				StartTimeValue -= Time.deltaTime;
				int timer_int = (int)StartTimeValue;
				startText.text = timer_int.ToString();
		...
		private void Move()
		{
			...
			//OWN PART, controls the character for proper jumping
			var position = transform.position;
			position += transform.forward * currentForwardSpeed;
			//how high the character jumps with one flap
			var JumpHeight = 2.5f;
			//how long it takes to get to the maximum height
			var JumpTime = 0.5f;
			//how long it stays on air
			var FlyTime = 1;
			if(_jumpTime >= 0)
            {
				if(_jumpTime < JumpTime)
                {
					position.y = (1-Mathf.Pow(1 - _jumpTime / JumpTime, 2)) * JumpHeight;
				}
				else if (_jumpTime < JumpTime + FlyTime)
				{
					position.y = JumpHeight;
				}
				else if (_jumpTime < JumpTime + FlyTime + JumpTime)
				{
					position.y = JumpHeight - Mathf.Pow((_jumpTime - FlyTime - JumpTime) / JumpTime, 2) * JumpHeight;
				}
				else
				{
					position.y = 0;
			...
			//position.y = Mathf.Sin(Mathf.Min(_jumpTime, 1) * Mathf.PI) * 2;
...
*/
		...
		private void UpdateMovementSpeed()
        {
            ...
            {
				currentSpeed = 0;
				SetPoseAiEnabled(false);
            }
			else
            {
				SetPoseAiEnabled(true);

				var pps = _armpumps.DistancePerSecond();
				if( pps > 0 )
                {
					currentSpeed += pps * Time.deltaTime * accelerationStrength;
                }
				else
                {
					//currentSpeed -= decelerationStrength * Time.deltaTime;
					currentSpeed *= Mathf.Clamp01(1 - decelerationStrength * Time.deltaTime);
                }

				currentSpeed = Mathf.Clamp(currentSpeed, minMovementSpeed, maxMovementSpeed);
			}
		...
		private void SetPoseAiEnabled( bool enable )
        {
			var directSource = (PoseAISourceDirect)_source;
			directSource.SetEnabled(enable);
        }
	}
	...
	public class StepCounter
	{
		...
		public void RegisterStep(float stepDistance)
		{
			...
			//steps per second measures the speed
